<%@ Page Title="" Language="VB" MasterPageFile="~/MemberPages/mpCampanha_Pascoa.master" AutoEventWireup="false" CodeFile="Campanha_Pascoa_Ovos.aspx.vb" Inherits="MemberPages_Campanha_Pascoa_Ovos" %>
...
<cc1:ToolTipMousePosition></cc1:ToolTipMousePosition>
                    ...
                    <asp:SqlDataSource ID="dsCrescimento" runat="server" 
                        ...
                        SelectCommand="SELECT Ano, perc FROM tblCampanhaGraficos WHERE (idCampanha = @Campanha) AND (idFilial = 99) AND (Cod = 2) AND (idGrafico = 1) AND AnoApuracao=@Ano">
                        <SelectParameters>
            ...
            <tr>
                <td align="center" valign="top" style="width: 303px; height: 164px;">
                    <dxwgv:ASPxGridView ID="gridMeta" runat="server" AutoGenerateColumns="False" 
                        ...
                        <SettingsText Title="Comparativo Ovos de Páscoa" />
                        <Columns>
                    ...
                    <asp:SqlDataSource ID="dsMeta" runat="server" 
                        ...
                        SelectCommand="SELECT tblCampanhaMeta.Descricao, tblCampanhaMeta.VendaAA, tblCampanhaMeta.Meta,   CASE WHEN tblCampanhaMeta.Real=0 THEN 0 WHEN tblCampanhaMeta.Meta = 0 THEN 0 ELSE tblCampanhaMeta.Real /  tblCampanhaMeta.Meta * 100 END AS Part
FROM tblCampanhaDesc INNER JOIN tblCampanhaMeta ON tblCampanhaDesc.idCampanha = tblCampanhaMeta.idCampanha AND tblCampanhaDesc.Cod = tblCampanhaMeta.Cod WHERE (tblCampanhaMeta.idCampanha = @Campanha) AND (tblCampanhaMeta.Ano = @Ano) AND (tblCampanhaMeta.Cod = 2) ORDER BY tblCampanhaMeta.idFilial">
                        <SelectParameters>
            ...
            <tr>
                <td>
                    <asp:SqlDataSource ID="dsSuper" runat="server" 
                        ...
                        SelectCommand="SELECT Ano, idFilial, 	ISNULL(QtdeAA,0) AS QtdeAA, Qtde, CASE WHEN ISNULL(QtdeAA,0)=0 THEN 0 WHEN Qtde=0 THEN 0 ELSE CAST(Qtde AS FLOAT) / CAST(QtdeAA AS FLOAT) * 100 - 100 END AS 'percCresQtde', ISNULL(VendaAA,0) AS VendaAA, Venda, CASE WHEN ISNULL(VendaAA,0)=0 THEN 0 WHEN Venda=0 THEN 0 ELSE CAST(Venda AS FLOAT) / CAST(VendaAA AS FLOAT) * 100 - 100 END AS 'percCresVenda',ISNULL(LucroAA,0) AS LucroAA, Lucro, Descricao, Ordem, 
CASE WHEN ISNULL(LucroAA,0)=0 THEN 0 WHEN ISNULL(VendaAA,0)=0 THEN 0 ELSE CAST(LucroAA AS FLOAT) / CAST(VendaAA AS FLOAT) * 100 END AS MargemAA, 
CASE WHEN Lucro=0 THEN 0 WHEN Venda=0 THEN 0 ELSE CAST(Lucro AS FLOAT) / CAST(Venda AS FLOAT) * 100 END AS Margem, 
CASE WHEN Lucro=0 THEN 0 WHEN Venda=0 THEN 0 WHEN ISNULL(LucroAA,0)=0 THEN 0 WHEN ISNULL(VendaAA,0)=0 THEN 0 ELSE (CAST(Lucro AS FLOAT) / CAST(Venda AS FLOAT) * 100 - CAST(ISNULL(LucroAA,0) AS FLOAT) / CAST(ISNULL(VendaAA,0) AS FLOAT) * 100) END AS Dif, 
CASE WHEN Lucro=0 THEN 0 WHEN ISNULL(LucroAA,0)=0 THEN 0 ELSE CAST(Lucro AS FLOAT) / CAST(LucroAA AS FLOAT) * 100 - 100 END AS 'percCresLucro' FROM tblCampanhaView WHERE (Cod = 2) AND (idCampanha = @idCampanha) AND (Ano = @Ano) ORDER BY Ordem">
                        <SelectParameters>
    ...
    <tr>
        <td>
            <dxwgv:ASPxGridView ID="ASPxGridView2" runat="server" AutoGenerateColumns="False" DataSourceID="dsTop10" EnableTheming="True" Theme="DevEx">
                ...
                <SettingsText Title="Ovos de Páscoa - Item Top 10" />
                <Styles>
                    <Header HorizontalAlign="Center">
                    </Header>
                    <Footer Font-Bold="True">
