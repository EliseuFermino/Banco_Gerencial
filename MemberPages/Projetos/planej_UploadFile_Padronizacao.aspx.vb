...
Partial Class MemberPages_planejUploadArquivo
    ...
    Protected Sub btnSubmit_Manutencao_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit_Manutencao.Click
        ...
        If Me.ASPxCheckBox1.Checked = False Then
            If File.Exists(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName)) Then
                File.Delete(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName))
                MyFile.PostedFile.SaveAs(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName))
                Call AtualizarArquivos(oProjeto.PathArquivo)
                Return
            Else
                MyFile.PostedFile.SaveAs(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName))
            End If
        Else
            If File.Exists(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName)) Then
                File.Delete(oProjeto.CaminhoArquivo(Me.cboProjeto.Value, MyFile.PostedFile.FileName))
                MyFile.PostedFile.SaveAs(oProjeto.CaminhoArquivo(999, MyFile.PostedFile.FileName))
                Call AtualizarArquivos(oProjeto.PathArquivo)
                Return
            Else
                MyFile.PostedFile.SaveAs(oProjeto.CaminhoArquivo(999, MyFile.PostedFile.FileName))
    ...
    Private Sub CarregarArquivos(ByVal iCaminhoDiretorio As String)
        ...
        Using conn As New SqlConnection(cnnStr)
            Using cmdTest As New SqlCommand("uspProjUploadFile_Padronizacao", conn)
                ...
                cmdTest.Parameters.AddWithValue("@Tipo", Me.cboTipo.Value)
                Try
                    conn.Open()
    ...
    Private Sub AtualizarArquivos(ByVal iCaminhoDiretorio As String)

        Using conn As New SqlConnection(cnnStr)
            Using cmdTest As New SqlCommand("uspProjUploadFileUpdate_Padronizacao", conn)
                cmdTest.CommandType = CommandType.StoredProcedure
                cmdTest.Parameters.AddWithValue("@nomeArquivo", Me.txtNomeArquivoAmigavel.Text)
                cmdTest.Parameters.AddWithValue("@linkFile", CStr(Trim(iCaminhoDiretorio & MyFile.PostedFile.FileName)))
                ...
                Try
                    ...
                    Me.gridView.DataBind()
                    UserMsgBox(Me, "O arquivo " & MyFile.PostedFile.FileName & " foi substituido com sucesso!!!")

                Catch ex As Exception
                    UserMsgBox(Me, "Módulo: AtualizarArquivos - " & ex.Message)
                    UserMsgBox(Me, "Houve um problema. O arquivo " & MyFile.PostedFile.FileName & " não foi substituido.")
                End Try
                conn.Close()
            End Using
        End Using
    ...
    Private Sub DeletarArquivos()
        ...
        Using conn As New SqlConnection(cnnStr)
            Using cmdTest As New SqlCommand("uspProjDeleteFile_Padronizacao", conn)
                cmdTest.CommandType = CommandType.StoredProcedure
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If Not IsPostBack Then oVem.AtualizarEstatisticaPrograma(44, User.Identity.Name)
    ...
    Protected Sub Page_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        CType(Master.FindControl("lblTitle"), Label).Text = "Padronização - Inmportar arquivos"
    End Sub
End Class
