<%@ Page Title="Mini Auditório RH - Agenda" Language="VB" MasterPageFile="~/MemberPages/Site.master" AutoEventWireup="true" CodeFile="CalendarioMiniAuditorioRH.aspx.vb" Inherits="MemberPages_Calendario_MiniAuditorioRH" Culture="pt-BR" uiCulture="pt-BR" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    <dxwschs:ASPxScheduler ID="ASPxScheduler1" runat="server" 
        ...
        ResourceDataSourceID="dsResource" Start="2013-05-22" Theme="Office2010Black" ActiveViewType="WorkWeek">
        <storage>
            <appointments>
...
<asp:SqlDataSource ID="dsResource" runat="server" 
    ...
    SelectCommand="SELECT [UniqueID], [ResourceID], [ResourceName], [Color], [Image], [CustomField1] FROM [ResourcesMiniAuditorioRH]">
</asp:SqlDataSource>
<asp:SqlDataSource ID="dsAppointment" runat="server" 
    ConnectionString="<%$ ConnectionStrings:gerContConn2008 %>" 
    DeleteCommand="DELETE FROM [AppointmentsMiniAuditorioRH] WHERE [UniqueID] = @UniqueID" 
    InsertCommand="INSERT INTO [AppointmentsMiniAuditorioRH] ([Type], [StartDate], [EndDate], [AllDay], [Subject], [Location], [Description], [Status], [Label], [ResourceID], [ReminderInfo], [RecurrenceInfo], [CustomField1]) VALUES (@Type, @StartDate, @EndDate, @AllDay, @Subject, @Location, @Description, @Status, @Label, @ResourceID, @ReminderInfo, @RecurrenceInfo, @CustomField1)" 
    oninserted="dsAppointment_Inserted" 
    SelectCommand="SELECT [UniqueID], [Type], [StartDate], [EndDate], [AllDay], [Subject], [Location], [Description], [Status], [Label], [ResourceID], [ReminderInfo], [RecurrenceInfo], [CustomField1] FROM [AppointmentsMiniAuditorioRH]" 
    UpdateCommand="UPDATE [AppointmentsMiniAuditorioRH] SET [Type] = @Type, [StartDate] = @StartDate, [EndDate] = @EndDate, [AllDay] = @AllDay, [Subject] = @Subject, [Location] = @Location, [Description] = @Description, [Status] = @Status, [Label] = @Label, [ResourceID] = @ResourceID, [ReminderInfo] = @ReminderInfo, [RecurrenceInfo] = @RecurrenceInfo, [CustomField1] = @CustomField1 WHERE [UniqueID] = @UniqueID">
    <DeleteParameters>
