<%@ Page Title="Agenda - Universidade Corporativa Condor" Language="VB" MasterPageFile="~/MemberPages/Site.master" AutoEventWireup="true" CodeFile="CalendarioCamila.aspx.vb" Inherits="MemberPages_Calendario_Camila" Culture="pt-BR" uiCulture="pt-BR" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" Runat="Server">
    <dxwschs:ASPxScheduler ID="ASPxScheduler1" runat="server" 
        ...
        ResourceDataSourceID="dsResource" Start="2013-05-22" Theme="Metropolis" ActiveViewType="WorkWeek">
        <storage>
            <appointments>
...
<asp:SqlDataSource ID="dsResource" runat="server" 
    ...
    SelectCommand="SELECT [UniqueID], [ResourceID], [ResourceName], [Color], [Image], [CustomField1] FROM [ResourcesCamila]">
</asp:SqlDataSource>
<asp:SqlDataSource ID="dsAppointment" runat="server" 
    ConnectionString="<%$ ConnectionStrings:gerContConn2008 %>" 
    DeleteCommand="DELETE FROM [AppointmentsCamila] WHERE [UniqueID] = @UniqueID" 
    InsertCommand="INSERT INTO [AppointmentsCamila] ([Type], [StartDate], [EndDate], [AllDay], [Subject], [Location], [Description], [Status], [Label], [ResourceID], [ReminderInfo], [RecurrenceInfo], [CustomField1]) VALUES (@Type, @StartDate, @EndDate, @AllDay, @Subject, @Location, @Description, @Status, @Label, @ResourceID, @ReminderInfo, @RecurrenceInfo, @CustomField1)" 
    oninserted="dsAppointment_Inserted" 
    SelectCommand="SELECT [UniqueID], [Type], [StartDate], [EndDate], [AllDay], [Subject], [Location], [Description], [Status], [Label], [ResourceID], [ReminderInfo], [RecurrenceInfo], [CustomField1] FROM [AppointmentsCamila]" 
    UpdateCommand="UPDATE [AppointmentsCamila] SET [Type] = @Type, [StartDate] = @StartDate, [EndDate] = @EndDate, [AllDay] = @AllDay, [Subject] = @Subject, [Location] = @Location, [Description] = @Description, [Status] = @Status, [Label] = @Label, [ResourceID] = @ResourceID, [ReminderInfo] = @ReminderInfo, [RecurrenceInfo] = @RecurrenceInfo, [CustomField1] = @CustomField1 WHERE [UniqueID] = @UniqueID">
    <DeleteParameters>
