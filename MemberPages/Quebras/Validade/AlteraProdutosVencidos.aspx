<%@ Page Title="Altera Produtos Vencidos" Language="VB" MasterPageFile="~/MemberPages/Site.master" AutoEventWireup="false" CodeFile="AlteraProdutosVencidos.aspx.vb" Inherits="MemberPages_Quebras_Validade_AlteraProdutosVencidos" %>
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <asp:SqlDataSource ID="dsDados" runat="server" ConnectionString="<%$ ConnectionStrings:DW_Condor_One_ConnectionString %>" 
                                   DeleteCommand="DELETE FROM [Vencidos].[tblProdutosVencidos] WHERE [idFilial] = @idFilial AND [Dia] = @Dia AND [dataVencimento] = @dataVencimento AND [Setor] = @Setor AND [EAN] = @EAN"  
                                   SelectCommand="SELECT * FROM [Vencidos].[tblProdutosVencidos] WHERE (([Dia] = @Dia) AND ([idFilial] = @idFilial))" 
                                   UpdateCommand="UPDATE [Vencidos].[tblProdutosVencidos] SET [numCorredor] = @numCorredor, [vlrPrecoVenda] = @vlrPrecoVenda, [Quantidade] = @Quantidade,  [Hora] = @Hora WHERE [idFilial] = @idFilial AND [Dia] = @Dia AND [dataVencimento] = @dataVencimento AND [Setor] = @Setor AND [EAN] = @EAN" 
                                   InsertCommand="INSERT INTO [Vencidos].[tblProdutosVencidos] ([idFilial], [Dia], [dataVencimento], [Setor], [numCorredor], [EAN], [vlrPrecoVenda], [Quantidade], [Usuario], [Hora]) VALUES (@idFilial, @Dia, @dataVencimento, @Setor, @numCorredor, @EAN, @vlrPrecoVenda, @Quantidade, @Usuario, @Hora)">
                    <DeleteParameters>
