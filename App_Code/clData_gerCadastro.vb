...
Public Class clData_gerCadastro
    ...
    Public Shared dbConnect As String = ConnectionStrings("gerCadastrosConnectionString").ConnectionString
    ...
    Public Shared Sub uspAnoMes(NomeProcedureStored As String, iAno As Integer, iMes As Integer)
    ...
    Public Shared Sub uspSecaoDias(ByVal NomeProcedureStored As String, ByVal iSecao As Int32, ByVal iDiaInicial As Date, ByVal iDiaFinal As Date, ByVal iUsuario As String)
    ...
    Public Function BuscaDiaDaSemana(ByVal iAno As String, ByVal iNumCampanha As String, ByVal iNumSemana As String) As Byte
        Dim con As New SqlClient.SqlConnection(dbConnect)
        Try

            Dim cmd1 As SqlCommand = New SqlCommand("uspBuscaDiasDaSemana_Campanha", con)
            cmd1.CommandType = CommandType.StoredProcedure
            cmd1.Parameters.Add("@ano", SqlDbType.SmallInt, ParameterDirection.Input).Value = iAno
            cmd1.Parameters.Add("@idCampanha", SqlDbType.SmallInt, ParameterDirection.Input).Value = iNumCampanha
            cmd1.Parameters.Add("@numSemana", SqlDbType.TinyInt, ParameterDirection.Input).Value = iNumSemana
            cmd1.Parameters.Add("@qtdeDias", SqlDbType.TinyInt)
            cmd1.Parameters("@qtdeDias").Direction = ParameterDirection.Output
            con.Open()
            cmd1.ExecuteNonQuery()
            BuscaDiaDaSemana = cmd1.Parameters("@qtdeDias").Value.ToString()

        Catch ex As Exception
            varDesc = "Classe: BuscaDiaDaSemana 6 - " & ex.Message
            Exit Function
        Finally
            con.Close()
        End Try
