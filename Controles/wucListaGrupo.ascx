...
<%@ Control Language="VB" AutoEventWireup="false" CodeFile="wucListaGrupo.ascx.vb" Inherits="Controles_wucListaGrupo" %>
...
<table>
    ...
    SelectCommand="SELECT idGrupo, GrupoDesc FROM Cadastros.tblCategoriaLinha WHERE (idCat = @idCat) ORDER BY Ordem">
        <SelectParameters>
    ...
    ConnectionString="<%$ ConnectionStrings:gerManagerConnectionString %>" 
    SelectCommand="SELECT idSubgrupo, SubGrupoDesc FROM Cadastros.tblCategoriaLinhaDet WHERE (idGrupo = @Grupo) ORDER BY Ordem">
        <SelectParameters>
